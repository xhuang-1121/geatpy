...
class MoeaAlgorithm(Algorithm): # 多目标优化算法模板父类
    ...
    def stat(self, population): # 分析记录，更新进化记录器，population为传入的种群对象，NDSet为当代的种群中的非支配个体集
        ...
        if len(feasible) > 0:
            ...
            if self.drawing == 2:
                # 绘制目标空间动态图
                self.ax = ea.moeaplot(population.ObjV, 'objective values', False, self.ax, self.currentGen, gridFlag = True)
            elif self.drawing == 3:
                # 绘制决策空间动态图
                self.ax = ea.varplot(population.Phen, 'decision variables', False, self.ax, self.currentGen, gridFlag = False)
