...
class CF1(ea.Problem): # 继承Problem父类
    ...
    def aimFunc(self, pop): # 目标函数
        ...
        x1 = Vars[:, [0]]
        J1 = np.array(range(2, self.Dim ,2))
        J2 = np.array(range(1, self.Dim ,2))
        f1 = x1   + 2 * np.mean((Vars[:, J1] - x1 ** (0.5 * (1 + 3 * (J1 + 1 - 2)/(self.Dim - 2))))**2, 1, keepdims = True)
        f2 = 1 - x1   + 2 * np.mean((Vars[:, J2] - x1 ** (0.5 * (1 + 3 * (J2 + 1 - 2)/(self.Dim - 2))))**2, 1, keepdims = True)
